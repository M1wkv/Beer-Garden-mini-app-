...
function updateCartCount(shouldAnimate = true) {
  ...
  scheduleHomeGridPositionUpdate(total > 0);
}

function scheduleHomeGridPositionUpdate(isVisible = Object.values(cart).some((item) => item.count > 0)) {
  updateHomeGridPosition(isVisible);
  setTimeout(() => updateHomeGridPosition(isVisible), 80);
  setTimeout(() => updateHomeGridPosition(isVisible), 260);
}

function updateHomeGridPosition(isVisible = Object.values(cart).some((item) => item.count > 0)) {
  const homeGrid = document.querySelector(".home-grid");

  if (!homeGrid || !elements.cartBtn) {
    return;
  }
  ...
  requestAnimationFrame(() => {
    ...
    const cartHeight = elements.cartBtn.getBoundingClientRect().height || parseFloat(rootStyle.getPropertyValue("--floating-height")) || 60;
    const gridGap = parseFloat(rootStyle.getPropertyValue("--home-grid-gap")) || 7;
    const nextBottom = Math.ceil(cartBottom + cartHeight + gridGap);
...
function isStoredCartExpired() {
  const lastActivity = parseInt(localStorage.getItem(CART_ACTIVITY_STORAGE_KEY), 10) || 0;
  return Boolean(lastActivity && Date.now() - lastActivity > CART_IDLE_TIMEOUT_MS);
}

function clearStoredCart() {
  cart = {};
  localStorage.removeItem(CART_STORAGE_KEY);
  localStorage.removeItem(CART_ACTIVITY_STORAGE_KEY);
}
...
function saveCart() {
  try {
    const hasItems = Object.values(cart).some((item) => item.count > 0);
    ...
    localStorage.setItem(CART_ACTIVITY_STORAGE_KEY, String(Date.now()));
  } catch (error) {
