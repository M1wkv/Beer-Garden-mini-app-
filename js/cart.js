function changeCount(name, delta) {
  const item = Object.values(menuData).flat().find((menuItem) => {
    return getField(menuItem, "name", "Name") === name;
  });
  ...
  cart[name].price = parsePrice(getField(item, "price", "Price"));
...
function openCart() {
  ...
  elements.header.textContent = "Ваш заказ";
...
function goBackFromCart() {
  ...
  elements.header.textContent = currentAPI === BAR_API ? "Барное меню" : "Меню";
