...
function updateCartCount(shouldAnimate = true) {
  ...
  scheduleHomeCartOffsetUpdate(total > 0);
}

function scheduleHomeCartOffsetUpdate(isVisible = Object.values(cart).some((item) => item.count > 0)) {
  updateHomeCartOffset(isVisible);
  setTimeout(() => updateHomeCartOffset(isVisible), 80);
  setTimeout(() => updateHomeCartOffset(isVisible), 260);
}
