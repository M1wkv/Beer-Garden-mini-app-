...
async function loadData(api) {
  ...
  try {
    ...
    if (firstCategory) {
      saveActiveCategory(firstCategory);
      showCategory(firstCategory);
      alignCategoryToStart(firstCategory);
    } else {
...
function rerenderMenuData(items) {
  ...
  if (activeCategory && menuData[activeCategory]) {
    const activeButton = Array.from(elements.categories.querySelectorAll(".category")).find((button) => {
      return button.textContent === activeCategory;
    });
    ...
    alignCategoryToStart(activeCategory);
    return;
  }
...
function alignCategoryToStart(category) {
  requestAnimationFrame(() => {
    ...
    if (categoryButton) {
      scrollCategoryToStart(categoryButton, "auto");
      return;
    }
